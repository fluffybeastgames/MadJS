...
function game_loop_client() {
  ...
  setTimeout(() => {
    window.requestAnimationFrame(() => game_loop_client());
  }, RENDER_REFRESH_TIME); // therefore each game loop will last at least tick_time ms
}
...
function add_slider(
  ...
  slider_range.addEventListener(
    "change",
    function () {
      console.log("I CHANGED A SLIDER");
      document.getElementById(
        `${id_prefix}_label`
      ).innerHTML = `${slider_range.value}`;
    },
    false
  );
...
function populate_new_game_overlay() {
  ...
  overlay_container_box.addEventListener("click", function (e) {
    e.stopPropagation();
  }); // prevent clicking on the new game box from closing the overlay
...
function page_load_behavior() {
  ...
  canvas.addEventListener(
    ...
    false
  ); //our main click handler function
  canvas.addEventListener(
    "contextmenu",
    function (event) {
...
class CellClient {
  ...
  draw_cell() {
    ...
    this.context.strokeRect(
      this.col * CellClient.width,
      this.row * CellClient.height,
      CellClient.width,
      CellClient.height
    );
    ...
    if (this.visible) {
      ...
      if (this.owner != null) {
        //} && this.entity != ENTITY_TYPE_ADMIRAL) { //If the spot is owned, draw a circle over it in the owner's color
        // this.draw_circle();
        // this.draw_outline();
        this.context.fillStyle = get_player_color(this.owner);
        this.context.fillRect(
          this.col * CellClient.width + 1,
          this.row * CellClient.height + 1,
          CellClient.width - 2,
          CellClient.height - 2
        );
      }
  ...
  draw_outline(color) {
    ...
    this.context.lineTo(
      x + CellClient.width - line_width / 2,
      y + line_width / 2
    );
    this.context.lineTo(
      x + CellClient.width - line_width / 2,
      y + CellClient.height - line_width / 2
    );
    this.context.lineTo(
      x + line_width / 2,
      y + CellClient.height - line_width / 2
  ...
  draw_star(num_points = 5) {
    // Draws an n-pointed star within the bounds of the cell, representing an Admiral or ship cell
    ...
    let fillColor =
      this.owner != null
        ? get_player_color(this.owner)
        : CellClient.neutral_entity_color;
  ...
  draw_arrow(dir, action) {
    // Draw an arrow over the cell to indicate a queued move
    let arrow_color = "#FFFFFF";
    if (action == ACTION_MOVE_ALL) {
      arrow_color = "#FF0000";
    } else if (action == ACTION_MOVE_HALF) {
      arrow_color = "#FFcc00";
    }
    ...
    // this.context.beginPath();
    else this.context.lineWidth = 1; //for some reason this needs to be called both before and after drawing the line, or the arrows won't render correctly
    ...
    this.context.lineTo(
      x_dest - arrow_head_length * Math.cos(angle - Math.PI / 6),
      y_dest - arrow_head_length * Math.sin(angle - Math.PI / 6)
    );
...
function create_client_cells(n_rows, n_cols) {
  // in the future this will only be defined on the client side
...
function render_board() {
  ...
  if (game_tick_local) {
    document.getElementById(
      "turn_counter"
    ).innerText = `Turn ${game_tick_local}`;
    document.getElementById(
...
function highlight_active_cell() {
  cells_client.forEach((cell) => {
    if (cell.row == active_cell[0] && cell.col == active_cell[1]) {
      ...
      if (move_mode == ACTION_MOVE_NORMAL) {
        cell.context.strokeStyle = "#FFFFFF";
      } else if (move_mode == ACTION_MOVE_ALL) {
        cell.context.strokeStyle = "#FF0000";
      } else if (move_mode == ACTION_MOVE_HALF) {
        cell.context.strokeStyle = "#FFcc00";
      }

      cell.context.strokeRect(
        cell.col * CellClient.width,
        cell.row * CellClient.height,
        CellClient.width,
        CellClient.height
      );
      ...
      cell.context.fillRect(
        (cell.col - 1) * CellClient.width,
        cell.row * CellClient.height,
        CellClient.width,
        CellClient.height
      );
...
function canvas_mouse_handler(event) {
  ...
  if (event.button == 0) {
    ...
    if (selection_changed) {
      ...
      //cycle through the options by repeatedly clicking on the same cell, makes it easier for users without mice to play
      switch (move_mode) {
        case ACTION_MOVE_NORMAL:
          move_mode = ACTION_MOVE_HALF;
          break;
        case ACTION_MOVE_HALF:
...
function wheel_handler(event) {
  ...
  if (event.deltaY > 0 && zoom_scale > MIN_SCALE) {
    // zoom out
    zoom_scale--;
  } else if (event.deltaY < 0 && zoom_scale < MAX_SCALE) {
    //zoom in
    zoom_scale++;
  }

  CellClient.height = Math.round(
    DEFAULT_CANVAS_HEIGHT * (zoom_scale / DEFAULT_ZOOM)
  );
  CellClient.width = Math.round(
    DEFAULT_CANVAS_WIDTH * (zoom_scale / DEFAULT_ZOOM)
  );
...
function select_cell_at(x, y) {
  // returns true if the active cell changed, and false if the cell was already selected
  let row, col, id;
...
function handle_key_press(key_key) {
  ...
  if (
    (key_key == "W" || key_key == "w" || key_key == "ArrowUp") &&
    active_cell[0] > 0
  ) {
    ...
    (key_key == "A" || key_key == "a" || key_key == "ArrowLeft") &&
    active_cell[1] > 0
  ) {
    // left
    ...
    (key_key == "S" || key_key == "s" || key_key == "ArrowDown") &&
    active_cell[0] < game_data.game.n_rows - 1
  ) {
    // down
    target_row = active_cell[0] + 1;
    target_col = active_cell[1];
    ...
    (key_key == "D" || key_key == "d" || key_key == "ArrowRight") &&
    active_cell[1] < game_data.game.n_cols - 1
  ) {
    // right
    target_row = active_cell[0];
    target_col = active_cell[1] + 1;
...
function undo_queued_move() {
  //undo last queued move and return active cell to it
...
function add_to_queue(source_row, source_col, target_row, target_col, dir) {
  ...
  if (move_mode == ACTION_MOVE_HALF) {
    move_mode = ACTION_MOVE_NORMAL;
  } // if we had right clicked to move half and half applied the half move in the above step, then we want to revert to normal movement mode
...
function client_receives_game_state_here(game_state_string) {
  ...
  if (cells_client.length > 0) {
    ...
    game_state_data.board.forEach((new_cell) => {
      if ("owner" in new_cell) {
        cells_client[new_cell.id].owner = new_cell.owner;
      }
      if ("troops" in new_cell) {
        cells_client[new_cell.id].troops = new_cell.troops;
      }
      if ("entity" in new_cell) {
        cells_client[new_cell.id].entity = new_cell.entity;
      }
      if ("terrain" in new_cell) {
        cells_client[new_cell.id].terrain = new_cell.terrain;
      }
      if ("visible" in new_cell) {
        cells_client[new_cell.id].visible = new_cell.visible;
      }
    });
    ...
    const table_body = scoreboard_data
      .map((value) => {
        let bg_color =
          value.admirals == 0 ? CellClient.neutral_entity_color : value.color; // remove the player's fleet color from the scoreboard if they're out of the game
