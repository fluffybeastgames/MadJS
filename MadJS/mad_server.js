...
class Game {
  ...
  tick() {
    ...
    if (this.game_on) {
      ...
      this.cells.forEach((cell) => {
        if (cell.owner != null) {
          if (
            cell.entity == ENTITY_TYPE_ADMIRAL &&
            this.game_tick_server % 2 == 0
          ) {
            // admiral grow every 2 turns
            cell.troops++;
          } else if (
            cell.entity == ENTITY_TYPE_SHIP &&
            this.game_tick_server % 16 == 0
          ) {
            cell.troops++;
          } else if (
            cell.entity == ENTITY_TYPE_SHIP_2 &&
            this.game_tick_server % 6 == 0
          ) {
            cell.troops++;
          } else if (
            cell.entity == ENTITY_TYPE_SHIP_3 &&
            this.game_tick_server % 2 == 0
          ) {
            cell.troops++;
          } else if (
            cell.entity == ENTITY_TYPE_SHIP_4 &&
            this.game_tick_server % 1 == 0
          ) {
            cell.troops++;
          } else if (
            ...
            // swamps drain every turn
            cell.troops--;
            if (cell.troops < 0) {
              //check if the swamp killed off the last troops in the cell
              cell.troops = 0;
              cell.owner = null;
            }
          } else if (this.game_tick_server % 25 == 0) {
            // regular owned cells grow every 25 turns
            cell.troops++;
          }
        }
      });
      ...
      for (let i = 0; i < this.players.length; i++) {
        let player;
        if (this.game_tick_server % 2 == 0) {
          // flip flop the turn order each round
          ...
          if (this.cells[cell_id_source].owner == move.queuer) {
            ...
            if (this.cells[cell_id_dest].terrain == TERRAIN_TYPE_MOUNTAIN) {
              ...
              if (move.action == ACTION_MOVE_NORMAL) {
                ...
                //right click
                troops_to_move = Math.floor(
                  this.cells[cell_id_source].troops / 2
                );
              }
              ...
              if (this.cells[cell_id_dest].owner == move.queuer) {
                ...
                // Otherwise invade the destination cell
                this.cells[cell_id_source].troops -= troops_to_move;
                this.cells[cell_id_dest].troops -= troops_to_move;
              ...
              if (this.cells[cell_id_dest].owner == move.queuer) {
                //either we owned it already or it was just taken over
                ...
                if (
                  [
                    ...
                    ENTITY_TYPE_SHIP_3,
                    ENTITY_TYPE_SHIP_4,
                  ].includes(this.cells[cell_id_source].entity)
                ) {
                  //} && move.action == ACTION_MOVE_ALL) {
                  this.try_to_move_ship(
                    cell_id_source,
                    cell_id_dest,
                    move.action
                  );
  ...
  try_to_move_ship(cell_id_source, cell_id_dest, action) {
    ...
    if (
      [
        ...
        ENTITY_TYPE_SHIP_4,
        null,
      ].includes(dest_entity) && // make sure we're not moving into an admiral
      [TERRAIN_TYPE_WATER, TERRAIN_TYPE_SWAMP].includes(dest_terrain)
    ) {
      // make sure we're able to put a ship here
    ...
    } else if (action == ACTION_MOVE_ALL) {
      // instead of abandoning ship, leave one troop behind
      this.cells[cell_id_source].troops += 1;
    }
  }
  ...
  distance_to_nearest_admiral(from_address) {
    // gets the Manhattan distance to the nearest admiral. Returns 999 if none found
    ...
    this.cells.forEach((cell) => {
      ...
      if (cell.entity == ENTITY_TYPE_ADMIRAL) {
        let distance =
          Math.abs(ref_row - cell.row) + Math.abs(ref_col - cell.col);
  ...
  spawn_admirals(starting_troops) {
    for (let i = 0; i < this.players.length; i++) {
      let not_found = true;
      while (not_found) {
        let rand_cell_id = Math.floor(
          Math.random() * this.num_rows * this.num_cols
        );
        if (
          this.cells[rand_cell_id].owner == null &&
          this.distance_to_nearest_admiral(rand_cell_id) > MIN_DISTANCE_ADMIRALS
        ) {
  ...
  send_game_state_to(player_id, emit_code) {
    ...
    let next_queue_id =
      this.players[player_id].queued_moves.length > 0
        ? this.players[player_id].queued_moves[player_id].id
        : -1; // if there are any items remaining in the queue, let them know which ones we've eliminated this turn. -1 will indicate to the client that the queue is empty
    ...
    this.cells.forEach((cell) => {
      if (should_be_visible(cell, player_id, fog_of_war_distance)) {
        ...
        if (cell.owner != null) {
          cell_string += `, "owner":${cell.owner}`;
        }
        if (cell.terrain != TERRAIN_TYPE_WATER) {
          cell_string += `, "terrain":${cell.terrain}`;
        }
        if (cell.entity != null) {
          cell_string += `, "entity":${cell.entity}`;
        }
        if (cell.troops != null) {
          cell_string += `, "troops":${cell.troops}`;
        }
        if (true) {
          cell_string += `, "visible":true`;
        }
...
class CellServer {
  ...
  neighbor(dir) {
    //returns the neighboring cell. If out of bounds, returns null
...
class HumanPlayer {
  ...
  admiral_count() {
    ...
    game.cells.forEach((cell) => {
      if (cell.owner == this.uid && cell.entity == ENTITY_TYPE_ADMIRAL) {
        counter++;
      }
    });
    return counter;
...
class Bot {
  ...
  take_move() {
    if (this.queued_moves.length < 1) {
      ...
      if (action_choice < 0.1) {
        this.attack_something();
      } else if (action_choice < 0.15) {
        this.gather_troops();
      }
      this.grow();
    }
    ...
    game.cells.forEach((cell) => {
      if (cell.owner != this.uid) {
        ...
        potential_origins.push({
          address: [cell.row, cell.col],
          weight: cell.troops - 1,
        }); //prioritize starting w/ larger piles of cells to increase the number of cells per move involved. troops-1 to ensure we ignore cells with fewer than 2 troops
      }
    });
        ...
        if (path) {
          for (let i = 0; i < path.length - 1; i++) {
            let new_move = {
              ...
              dir: "n/a",
              queuer: this.uid,
              target_row: path[i + 1][0],
              target_col: path[i + 1][1],
      ...
      if (cell.owner == this.uid) {
        switch (cell.entity) {
          ...
          case null:
            ...
            break; //prioritize starting w/ larger piles of cells to increase the number of cells/move that end up in the entity. troops-1 to ensure we ignore cells with fewer than 2 troops
        }
      }
    });
        ...
        if (path) {
          path.forEach((cell) => {
            let new_move = {
              ...
              dir: "n/a",
              queuer: this.uid,
              target_row: target_address.address[0],
              target_col: target_address.address[1],
              action: ACTION_MOVE_NORMAL,
            };
  ...
  grow() {
    // a bot behavior that emphasizes growth over safey or combat. However, it will probably try to take over admirals and ships, given the chance
    function eval_potential_move(cell, target, this_uid, troop_count) {
      ...
      if (target.terrain == TERRAIN_TYPE_MOUNTAIN) {
        return [0, ACTION_MOVE_NONE];
      } // don't try to grow into mountains (not yet, anyway)
      if (cell.troops <= 1) {
        return [0, ACTION_MOVE_NONE];
      } // don't try growing if you don't have any troops to spare

      let weight = 0;
      let move_mode = ACTION_MOVE_NORMAL;
      if (target.owner == this_uid) {
        weight += 1;
      }
      if (target.owner == null) {
        weight += 15;
      }
      if (target.owner != this_uid && target.troops < cell.troops + 1) {
        weight += 10;
      }
      if (
        target.owner != this_uid &&
        target.troops < cell.troops + 1 &&
        target.entity == ENTITY_TYPE_ADMIRAL
      ) {
        weight += 40;
      } //strongly prioritize capturing enemy admirals
      if (
        target.owner != this_uid &&
        target.troops < cell.troops + 1 &&
        target.entity == ENTITY_TYPE_SHIP
      ) {
        weight += 10;
      ...
      if (
        cell.terrain == TERRAIN_TYPE_SWAMP &&
        target.terrain == TERRAIN_TYPE_WATER
      ) {
        weight += 25;
        move_mode = ACTION_MOVE_ALL;
      }
      if (target.terrain == TERRAIN_TYPE_SWAMP) {
        weight -= 15;
      }
    ...
    game.cells.forEach((cell) => {
      if (cell.owner == this.uid) {
        ...
        if (neighbor_left) {
          let result = eval_potential_move(
            cell,
            neighbor_left,
            this.uid,
            troop_count
    ...
    for (let i = 0; i < num_moves_to_queue; i++) {
      if (potential_moves.length > 0) {
        ...
        if (result) {
          ...
          potential_moves = potential_moves.filter(
            (item) => item.cell !== result.cell
          );
        }
      }
    }
...
function request_new_game(game_data_json) {
  ...
  let fog_of_war =
    "fog_of_war" in game_data ? game_data.fog_of_war : Math.random() > 0.5;
  let n_bots =
    "n_bots" in game_data
      ? game_data.n_bots
  ...
  let water_weight =
    "water_weight" in game_data ? game_data.water_weight : 10 + Math.random();
  let mountain_weight =
    "mountain_weight" in game_data
      ? game_data.mountain_weight
      : 1 + Math.random();
  let swamp_weight =
    "swamp_weight" in game_data
      ? game_data.swamp_weight
      : 0.1 + Math.random() / 4;
  let ship_weight =
    "ship_weight" in game_data
      ? game_data.ship_weight
      : 0.2 + Math.random() / 2;

  const bot_color_options = [
    "#C50F1F",
    "#C19C00",
    "#881798",
    "#E74856",
  ...
  let player_name =
    "player_name" in game_data ? game_data.player_name : "Player One";
...
function get_owned_neighbors(cell, player_id, fog_of_war_distance) {
  // Returns the number of adjacent cells owned by the provided player_id. Normally, this is used to determine if a cell should be visible to said user
  let num_neighbors = 0;
  ...
  cells_to_check.forEach((cell) => {
    if (
      cell[0] >= 0 &&
      cell[1] >= 0 &&
      cell[0] < game.num_rows &&
      cell[1] < game.num_cols
    ) {
      num_neighbors +=
        get_cell_by_coords(cell[0], cell[1]).owner == player_id ? 1 : 0;
    }
  });
  return num_neighbors;
}

function get_cell_by_coords(row, col) {
  // Returns the server cell object at the given row and column
...
function weighted_choice(arr_options) {
  ...
  arr_options.forEach((option) => {
    total_weight += Math.max(option.weight, 0);
  }); // sum up the individual weights to determine the scale of our randrange

  let rand_weight = Math.random() * total_weight;

  let traversed_weight = 0,
    arr_pos = -1;
...
io.on("connection", (socket) => {
  ...
  socket.on("undo_queued_move", (player_id, popped_item_id) => {
    ...
    while (game.players[player_id].queued_moves.length > 0 && not_caught_up) {
      if (
        game.players[player_id].queued_moves[
          game.players[player_id].queued_moves.length - 1
        ].id >= popped_item_id
      ) {
